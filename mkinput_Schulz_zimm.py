...
def generateInputFile(nbeadsInput, monomers_per_polymer_input, npoly_input, minsep_input, dens_input, bond_length_input,
                      p_branch_end_input, filename, nmin, nmax, schulz_zimm=True, Flory_Schulz=False ):
    class Schulz_zimm(rv_continuous):
        def _pdf(self,x, z, Mn):
            return ((z**(z+1)/gamma(z+1))*(x**(z-1)/Mn**(z))*np.exp(-z*x/Mn))
    z = 2.95
    Mn = 17.5
    my_cv = Schulz_zimm(a=nmin, b=nmax, seed = 10, name='Schulz_Zimm')
    N = 3600
    my_cv = st.gamma(scale=z)
    print("This is my", my_cv)
    X = my_cv.rvs(size=N, a= Mn)
    monomers_per_polymer = X
    polydispersity = True
    list_mon_ = list(monomers_per_polymer)
    list_mon = list(map(int, list_mon_))
    monomers_per_polymer = list_mon
    print(list_mon)
    print("length of the polymer", len(list_mon))
    #print("Total number of beads",sum(monomers_per_polymer))
    ...
    with open("composition.csv", 'w') as file:
        file.write("No_of_monomers, number_fraction, weight_fraction")
        file.write("\n")
        for i in data.index:
    ...
    if not polydispersity:
        ...
        nbonds = np.sum(monomers_per_polymer) - len(monomers_per_polymer)
    print()
...
generateInputFile(1,0,3600,1,0.85,0.97,0,'input.lammps',nmin = 3, nmax = 313)
