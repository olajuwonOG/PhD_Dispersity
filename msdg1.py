...
#g1(t) - for monomers in selected selected_chains
...
import warnings
# INPUT PARAMETERs
nconf =  200 #20030 # number of configurations to take data for
#stepspacing = 10000000
#polylength= 30  #now based on type and not polymer number
npoly =    4000 #need this for com calc
#numpy.seterr(invalid='ignore')
#selected_mol_ids = [i for i in range(100)]
lower_chain_value, higher_chain_value = 55, 65
counter = 0
warnings.filterwarnings("ignore", category=RuntimeWarning)

def msdcalc():
    global msdvec1, msdvec2, msdvec3, msdvec4, msdvec5, msdvec6, msdvec7, msdvec8, timestep, msd, xx, yy, zz, x0, y0, z0, x0com, y0com, z0com, xcom, ycom, zcom, step, num1, num2, num3, num4, num5, num6, npoly, selected_mol_ids
    global natoms, typea,msdcall, num_counts, atom_id_mol_id_dict, match_list, lower_chain_value, higher_chain_value
...
##    ninmol=zeros(numatoms, int)
    ...
    for ii in range(1,npoly+1): #msd loop for chains to calculate their center of mass
        if ii in selected_mol_ids:
            ...
            r2 = xr*xr + yr*yr + zr*zr
            msd[7] = msd[7]+r2
    #print("counter total is ", counter)
    msd[1]=msd[1]/len(match_list)
    msd[2]=msd[2]/num2
    msd[3]=msd[3]/num3
    msd[4]=msd[4]/num4
    msd[5]=msd[5]/num5
    ...
    msdvec7[msdcall]=msdvec7[msdcall]+msd[7] #msd vector for g3(t) specific
...
def wholecalculation():
    global msdvec1, msdvec2, msdvec3, msdvec4, msdvec5, msdvec6,msdvec7, msdvec8, timestep, msd, xx, yy, zz, x0, y0, z0, x0com, y0com, z0com, xcom, ycom, zcom, step, num1, num2, num3, num4, num5, num6, npoly
    global skip
    global natoms, typea,msdcall, atom_id_mol_id_dict, selected_mol_ids, match_list

    infiles = ['production.dump'] #PSsorted.lammpstrj
    file = 'msdcm_R1skip%dg1.csv'% skip
    dummy=0
    ...
    natoms = 0
    num_counts = {} #dictionary to get key (chain id) and value (length of the chain)
    ...
    for loopnum in range(0,1): #read starting timestep
        ...
        for j in range(1,dim):
            ...
            k=int(ii)
            atom_id_mol_id_dict[k] = int(molj) #key value pair; key = atom_id and value = molj/chain_id
        ...
        for key, value in num_counts.items():
            #if  value == 140:
            if lower_chain_value <= value <= higher_chain_value:
            #if  10 < value < 215:
        ...
        # match_list = []
        #
        # for key, value in atom_id_mol_id_dict.items():
        #     if value in selected_mol_ids:
        ...
        print("\n")
        #print("total atoms(k) and chain ids/molid are", atom_id_mol_id_dict)
        print("\n")
        print("length of the dictionary -- equals total atoms", len(atom_id_mol_id_dict))
        print("\n")
        ...
        for kconf in range(istart,nconf):
          try:
            ...
            ycm=ycm/natoms
            zcm=zcm/natoms"""
            for j in range(1,dim): #position of each bead relative to the center of mass at time, t > initial time ----Changes to made for g2(t)
            ...
            #OUT.close()
            #except: break
            print(("%7i %7i %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f\n" % (istep,step,msd[1],msd[2],msd[3],msd[4],msd[5],msd[6],msd[7], msd[8])))
          except:break
            #print("code has been corrupted")
        fileinput.close()

    # end of loop
    OUT = open(file, 'w')
    #OUT.write("msd\n")
    OUT.write("step, msd_type1, msd_type2, msd_type3, msd_type4, msd_type5, msd_type6, msd_COMspecific\n")
